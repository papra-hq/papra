...
export async function promptForOrganization({
  ...
  const selectedOrgId = await exitOnCancel(
    p.select({
...
export async function getOrganizationId({
  ...
  apiClient: Client;
  argOrganizationId?: string;
}) {
  const { organizations } = await exitOnError(apiClient.listOrganizations(), {
    errorContext: 'Failed to fetch organizations.',
  });
