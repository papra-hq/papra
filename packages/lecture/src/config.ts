...
export function parseConfig({ rawConfig = {} }: { rawConfig?: PartialExtractorConfig } = {}): {
  config: ExtractorConfig;
} {
  const languages = (rawConfig.tesseract?.languages ?? []).filter(
  ...
  if (invalidLanguages.length > 0) {
    throw new Error(
      `Invalid languages for tesseract: ${invalidLanguages.join(', ')}. Valid languages are: ${ocrLanguages.join(', ')}`,
    );
  }
