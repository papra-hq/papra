...
describe('extractors registry', () => {
  describe('getExtractor', () => {
    test('gets the extractor for a given mimeType', () => {
      ...
      expect(
        getExtractor({ mimeType: 'a/b', extractors: extractorDefinitions }).extractor?.name,
      ).to.eql('1');
      expect(
        getExtractor({ mimeType: 'a/c', extractors: extractorDefinitions }).extractor?.name,
      ).to.eql('2');
      expect(getExtractor({ mimeType: 'e/f', extractors: extractorDefinitions })).to.eql({
        extractor: undefined,
      });
    });
  });
    ...
    describe('non-regs', () => {
      ...
      test('all extractors have non empty mimeTypes array', () => {
        extractorDefinitions.forEach((extractor) => {
          expect(Array.isArray(extractor.mimeTypes)).to.eql(
            true,
            'All extractors must have a mimeTypes array',
          );
          expect(extractor.mimeTypes.length).to.be.greaterThan(
            0,
            'All extractors must have at least one mimeType',
          );
        });
      });
      ...
      test('all extractors have a unique mimeType', () => {
        ...
        expect(new Set(mimeTypes).size).to.eql(
          mimeTypes.length,
          'All extractors must have a unique mimeType',
        );
      });
    });
  });
