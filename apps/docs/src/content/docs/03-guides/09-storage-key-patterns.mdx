...
The generated storage key is stored in the database alongside each document record. By default, **changing the pattern configuration only affects newly uploaded documents**. Existing documents retain their original storage keys unless you enable synchronization or explicitly migrate them.

<Aside type="tip">
Use the [storage migration guide](/guides/migrate-document-storage/) to reprocess existing documents in bulk or move them to another storage backend.
</Aside>
...
<Aside type="caution">
If you switch from the legacy system to the pattern-based system on an existing instance, existing documents keep their legacy storage keys and remain accessible. With synchronization enabled, they adopt the configured pattern on the next update that triggers synchronization.
</Aside>

### Synchronizing Storage Keys on Document Updates

To reflect document renames in the underlying storage, enable synchronization alongside pattern-based storage:

```bash
DOCUMENT_STORAGE_USE_LEGACY_STORAGE_KEY_DEFINITION_SYSTEM=false
DOCUMENT_STORAGE_KEY_PATTERN={{organization.id}}/{{document.name}}
DOCUMENT_STORAGE_KEY_SYNC_ENABLED=true
```

The configuration option is `documentsStorage.pattern.isStorageKeySyncEnabled` and defaults to `false`. When enabled, qualifying document updates trigger pattern re-evaluation using the document's current metadata. Currently, synchronization is triggered by document name or document date changes. Papra copies the stored file to the new key, updates its database reference, and removes the old file. Encrypted files retain their encryption metadata.

Enabling synchronization does not immediately move existing files: it applies to subsequent updates that trigger synchronization.

When synchronization is triggered, all expressions are re-evaluated, including `{{currentDate.*}}` and `{{random}}`. These expressions do not trigger synchronization on their own. Synchronization runs asynchronously on a best-effort basis; failures are logged without automatic retries. Failed copies or database updates retain the original file, while failed old-file cleanup leaves the new key usable.
...
| `document.name` | The document name (sanitized for safe filesystem use) | `invoice-2025.pdf` |
| `document.date` | The document date in ISO 8601 format, or `no-date` when unset | `2025-06-15T00:00:00.000Z` |
| `document.createdAt` | The document's creation timestamp in ISO 8601 format | `2025-06-15T14:30:00.000Z` |
| `organization.id` | The organization identifier | `org_123456789012345678901234` |
| `organization.name` | The current organization name (sanitized for safe filesystem use) | `Acme Finance` |
...
<Aside type="note">
The `document.name` and `organization.name` expressions automatically sanitize names by replacing unsafe filesystem characters with underscores (`_`).
</Aside>

### Organization Names and Renames

Use `organization.name` for readable organization folders. Organization names are not unique, so include `organization.id` to keep folders distinct:

```bash
DOCUMENT_STORAGE_KEY_PATTERN={{organization.id}}-{{organization.name}}/{{document.name}}
```

Renaming an organization does not move its existing documents, even when storage-key synchronization is enabled. Their persisted keys remain usable. New uploads use the current organization name. With synchronization enabled, a later document name or document date change re-evaluates the entire pattern and uses the current organization name too. An explicit [storage migration](/guides/migrate-document-storage/) also uses the current name.

Papra looks up the organization name only when the pattern references `organization.name`.

## Available Transformers

Both `document.date` and `document.createdAt` support the `formatDate` transformer:

```
{{organization.id}}/{{document.date | formatDate "{yyyy}/{MM}"}}/{{document.name}}
```

If the document date is unset, the expression resolves to `no-date`, even with date formatting. Use `default` after formatting to customize the fallback, for example `{{document.date | formatDate | default "undated"}}`.

The `document.createdAt` value stays fixed on subsequent storage-key synchronizations. When storage-key synchronization is enabled, setting, changing, or clearing the document date also triggers synchronization.
...
| `DOCUMENT_STORAGE_KEY_SYNC_ENABLED` | Synchronize storage keys on document updates when pattern-based storage is enabled | `false` |
