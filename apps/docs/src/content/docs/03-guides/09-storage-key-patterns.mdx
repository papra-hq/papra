...
| `organization.name` | The organization name (sanitized for safe filesystem use) | `My Organization` |
...
<Aside type="note">
The `document.name` and `organization.name` expressions automatically sanitize their values by replacing unsafe filesystem characters with underscores (`_`).
</Aside>
...
### Organize by Organization Name

```bash
DOCUMENT_STORAGE_KEY_PATTERN={{organization.name}}/{{document.name}}
# Result: My Organization/invoice-2025.pdf
```
