...
This guide explains how to configure LLM providers in Papra. Once a provider is configured, it can be used to power AI features like [document auto tagging](/guides/auto-tagging) and [AI document extraction](/guides/ai-extraction).
...
Individual features can override this. For example, auto tagging reads `AUTO_TAGGING_MODEL` first and falls back to `AI_DEFAULT_MODEL` if it is not set. Document extraction reads `AI_EXTRACTION_MODEL` the same way.
