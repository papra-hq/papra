...
If you get "too many redirects" (check log from wrangler), check that your domain is configured with SSL/TLS Encryption as Full (strict). 
Cloudflare dashboard → (domain) → SSL/TLS → Overview → set encryption mode to Full (strict) or at minimum Full. Save, then re-send a test email.
