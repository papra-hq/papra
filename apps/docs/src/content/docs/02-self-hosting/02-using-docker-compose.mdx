...
import SecretKeyGenerator from '../../../components/random-hex-key-generator.astro';
...
2. Generate an Auth Secret
    ...
    Papra requires an `AUTH_SECRET` to run. Generate a secure one using the generator below or the OpenSSL command:

    <Tabs>
      <TabItem label="Key generator">
        <SecretKeyGenerator byteLength={48} />
      </TabItem>
    ...
    Keep the value handy, you will set it as an environment variable in the next step.

3. Create Docker Compose file

    Create a file named `docker-compose.yml` with the following content (replacing `<your-auth-secret>` with the value generated in the previous step):
