...
import { getMimeTypeForExtension } from '@/modules/lib/file/file.models';
import { fileToBase64DataUrl } from '@/modules/lib/file/file.services';
...
export async function processScannedImages({ imageUris, format, baseName }: { imageUris: string[]; format: ScanOutputFormat; baseName: string }): Promise<{ localDocuments: LocalDocument[] }> {
  ...
  // This should not happen due to typing, but safeguard
  throw new Error(`Unsupported scan output format: ${format as string}`);
}
