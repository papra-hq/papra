...
export const documentFileEncryptionMigration = {
  ...
  up: async ({ db }) => {
    ...
    const statements = [
      ...(!hasColumn('file_encryption_key_wrapped')
        ? [sql`ALTER TABLE documents ADD COLUMN file_encryption_key_wrapped TEXT`]
        : []),
      ...(!hasColumn('file_encryption_kek_version')
        ? [sql`ALTER TABLE documents ADD COLUMN file_encryption_kek_version TEXT`]
        : []),
      ...(!hasColumn('file_encryption_algorithm')
        ? [sql`ALTER TABLE documents ADD COLUMN file_encryption_algorithm TEXT`]
        : []),
