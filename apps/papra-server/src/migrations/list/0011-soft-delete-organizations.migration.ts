...
export const softDeleteOrganizationsMigration = {
  ...
  up: async ({ db }) => {
    ...
    const statements = [
      ...(hasColumn('deleted_by')
        ? []
        : [sql`ALTER TABLE "organizations" ADD "deleted_by" text REFERENCES users(id);`]),
      ...(hasColumn('deleted_at')
        ? []
        : [sql`ALTER TABLE "organizations" ADD "deleted_at" integer;`]),
      ...(hasColumn('scheduled_purge_at')
        ? []
        : [sql`ALTER TABLE "organizations" ADD "scheduled_purge_at" integer;`]),
