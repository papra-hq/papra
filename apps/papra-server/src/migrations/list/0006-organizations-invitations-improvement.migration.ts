...
export const organizationsInvitationsImprovementMigration = {
  ...
  up: async ({ db }) => {
    await db.batch([
      db.run(
        sql`ALTER TABLE "organization_invitations" ALTER COLUMN "role" TO "role" text NOT NULL`,
      ),
      db.run(
      ...
      db.run(
        sql`ALTER TABLE "organization_invitations" ALTER COLUMN "status" TO "status" text NOT NULL DEFAULT 'pending'`,
      ),
    ]);
  },
  ...
  down: async ({ db }) => {
    await db.batch([
      ...
      db.run(
        sql`ALTER TABLE "organization_invitations" ALTER COLUMN "status" TO "status" text NOT NULL`,
      ),
    ]);
  },
} satisfies Migration;
