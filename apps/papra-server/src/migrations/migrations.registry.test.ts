  ...
  describe('migrations', () => {
    ...
    test('regression test of the database state after running migrations, update the snapshot when the database state changes', async () => {
      ...
      expect(await serializeSchema({ db })).toMatchInlineSnapshot(`
        ...
        CREATE TABLE "organization_settings" ( "id" text PRIMARY KEY NOT NULL, "created_at" integer NOT NULL, "updated_at" integer NOT NULL, "organization_id" text NOT NULL, "ai_auto_tagging_enabled" integer, "ai_auto_tagging_can_create_new_tags" integer, "ai_auto_tagging_max_tags" integer, "ai_auto_tagging_model_id" text, ai_extraction_enabled integer, ai_extraction_extract_date integer, ai_extraction_extract_custom_properties integer, ai_extraction_rename_documents integer, ai_extraction_filename_pattern text, FOREIGN KEY ("organization_id") REFERENCES "organizations"("id") ON UPDATE cascade ON DELETE cascade, UNIQUE("organization_id") );
