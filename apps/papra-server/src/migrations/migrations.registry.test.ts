  ...
  describe('migrations', () => {
    ...
    test('regression test of the database state after running migrations, update the snapshot when the database state changes', async () => {
      ...
      expect(await serializeSchema({ db })).toMatchInlineSnapshot(`
        ...
        CREATE TABLE "organizations" ( "id" text PRIMARY KEY NOT NULL, "created_at" integer NOT NULL, "updated_at" integer NOT NULL, "name" text NOT NULL, "customer_id" text, "deleted_by" text, "deleted_at" integer, "scheduled_purge_at" integer, FOREIGN KEY ("deleted_by") REFERENCES "users"("id") ON UPDATE cascade ON DELETE set null );
