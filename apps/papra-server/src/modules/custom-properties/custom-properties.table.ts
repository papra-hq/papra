...
export const customPropertyDefinitionsTable = sqliteTable('custom_property_definitions', {
  ...
  type: text('type', {
    enum: CUSTOM_PROPERTY_TYPES_LIST as NonEmptyArray<CustomPropertyType>,
  }).notNull(),
  config: text('config', { mode: 'json' }).$type<Record<string, unknown>>(),
  displayOrder: integer('display_order').notNull().default(0),
});

export const documentCustomPropertyValuesTable = sqliteTable('document_custom_property_values', {
  ...
  propertyDefinitionId: text('property_definition_id')
    .notNull()
    .references(() => customPropertyDefinitionsTable.id, {
      onDelete: 'cascade',
      onUpdate: 'cascade',
    }),
  ...
  selectOptionId: text('select_option_id').references(() => customPropertySelectOptionsTable.id, {
    onDelete: 'cascade',
    onUpdate: 'cascade',
  }),
  ...
  relatedDocumentId: text('related_document_id').references(() => documentsTable.id, {
    onDelete: 'cascade',
    onUpdate: 'cascade',
  }),
});
