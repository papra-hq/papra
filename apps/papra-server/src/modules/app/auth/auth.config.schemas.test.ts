...
describe('auth config schemas', () => {
  describe('forbiddenEmailDomainsSchema', () => {
    test('create a set of domain from a comma separated string or an array of strings', () => {
      expect(v.parse(forbiddenEmailDomainsSchema, 'example.com, test.com')).to.eql(
    ...
    test('duplicate domains are removed', () => {
      expect(v.parse(forbiddenEmailDomainsSchema, 'example.com, test.com, example.com')).to.eql(
        new Set(['example.com', 'test.com']),
      );

      expect(
        v.parse(forbiddenEmailDomainsSchema, ['example.com', 'test.com', 'example.com']),
      ).to.eql(new Set(['example.com', 'test.com']));
    });

    test('invalid domains are rejected', () => {
      expect(() =>
        v.parse(forbiddenEmailDomainsSchema, 'example.com, inVAlid-domain, test.com'),
      ).to.throw('Invalid domain: Received "invalid-domain"');

      expect(() => v.parse(forbiddenEmailDomainsSchema, ['@example.com'])).to.throw(
