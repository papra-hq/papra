...
import {
  createInMemoryKvStoreDriver,
  IN_MEMORY_KV_STORE_DRIVER_NAME,
} from './in-memory/in-memory.kv-store-driver';
import {
  createLibsqlKvStoreDriver,
  LIBSQL_KV_STORE_DRIVER_NAME,
} from './libsql/libsql.kv-store-driver';
...
export const kvStoreDriverNames = Object.keys(
  kvStoreDriverFactories,
) as (keyof typeof kvStoreDriverFactories)[];

export type KvStoreDriverName = keyof typeof kvStoreDriverFactories;

export function getKvStoreDriverFactory({
  driverName,
}: {
  driverName: KvStoreDriverName;
}): KvStoreDriverFactory {
