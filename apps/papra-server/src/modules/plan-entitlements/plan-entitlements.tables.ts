...
export const planEntitlementsTable = sqliteTable(
  ...
  (table) => [
    // To ensure entitlements are unique per user
    unique('user_plan_entitlements_user_id_type_unique').on(table.userId, table.type),
  ],
);
