    ...
    test('update a link lastAccessedAt only if the last access was more than 1min ago', () => {
      expect(
        shouldTouchShareLinkLastAccessedAt({
          lastAccessedAt: Temporal.Instant.from('2026-05-12T11:58:00Z'),
          clock,
        }),
      ).to.eql(true);
      expect(
        shouldTouchShareLinkLastAccessedAt({
          lastAccessedAt: Temporal.Instant.from('2026-05-12T11:59:00Z'),
          clock,
        }),
      ).to.eql(false);
      expect(
        shouldTouchShareLinkLastAccessedAt({
          lastAccessedAt: Temporal.Instant.from('2026-05-12T11:59:01Z'),
          clock,
        }),
      ).to.eql(false);
