  ...
  describe('fsStorageDriver', async () => {
    runDriverTestSuites({
      createDriver: async () => {
        ...
        return {
          driver: fsStorageDriverFactory({ root: tmpDirectoryPath }),
    ...
    describe('saveFile', () => {
      test('persists the file to the filesystem', async () => {
        ...
        const fsStorageDriver = fsStorageDriverFactory({ root: tmpDirectoryPath });
        ...
        const fileExists = await fs.promises
          .access(storagePath, fs.constants.F_OK)
          .then(() => true)
          .catch(() => false);
    ...
    describe('getFileStream', () => {
      test('get a readable stream of a stored file', async () => {
        ...
        const { fileStream } = await fsStorageDriver.getFileStream({
          storageKey: 'org_1/text-file.txt',
        });
      ...
      test('an error is raised if the file does not exist', async () => {
        ...
        await expect(
          fsStorageDriver.getFileStream({ storageKey: 'org_1/text-file.txt' }),
    ...
    describe('deleteFile', () => {
      test('deletes a stored file', async () => {
        ...
        const fileInitiallyExists = await fs.promises
          .access(path.join(tmpDirectoryPath, 'org_1/text-file.txt'), fs.constants.F_OK)
          .then(() => true)
          .catch(() => false);
      ...
      test('when the file does not exist, an error is raised', async () => {
        ...
        await expect(
          fsStorageDriver.deleteFile({ storageKey: 'org_1/text-file.txt' }),
        ).rejects.toThrow(createFileNotFoundError());
      });
    });
  });
