...
import { coercedStorageKeyEncryptionKeysListSchema } from './storage-encryption.schemas';

describe('storage-encryption schemas', () => {
  describe('storageKeyEncryptionKeysSchema', () => {
    test('the user can provide a comma separated list of 32 bytes long hex strings env variable', () => {
      expect(
        v.parse(
          coercedStorageKeyEncryptionKeysListSchema,
        ...
        },
        {
          version: '2',
          key: Buffer.from(
            'abdc13ff846337ef514a62d7d2cd2aa3b517d957ab7c825b8de0c7678f17a843',
            'hex',
          ),
        },
      ]);
    ...
    test('no keys provided should not raise an error', () => {
      expect(v.parse(coercedStorageKeyEncryptionKeysListSchema, undefined)).to.eql(undefined);
    });

    test('keys should be 32 bytes long hex strings', () => {
      expect(() => v.parse(coercedStorageKeyEncryptionKeysListSchema, '0deba553')).to.throw();
