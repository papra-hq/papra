...
export const taggingRulesTable = sqliteTable('tagging_rules', {
  ...
  conditionMatchMode: text('condition_match_mode')
    .notNull()
    .default('all')
    .$type<ConditionMatchMode>(),
});

export const taggingRuleConditionsTable = sqliteTable('tagging_rule_conditions', {
  ...createPrimaryKeyField({ prefix: 'trc' }),
  ...createTimestampColumns(),

  taggingRuleId: text('tagging_rule_id')
    .notNull()
    .references(() => taggingRulesTable.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
...
export const taggingRuleActionsTable = sqliteTable('tagging_rule_actions', {
