...
export function aggregateTaggingRules({
  ...
  const taggingRulesRecord = rawTaggingRules.reduce(
    (acc, rawTaggingRule) => {
      ...
      if (rawTaggingRule.tagging_rule_conditions) {
        acc[taggingRuleId].conditions[rawTaggingRule.tagging_rule_conditions.id] =
          rawTaggingRule.tagging_rule_conditions;
      }
    ...
    {} as Record<
      string,
      TaggingRule & {
        conditions: Record<string, TaggingRuleCondition>;
        actions: Record<string, TaggingRuleAction & { tag: Tag | null }>;
      }
    >,
  );

  return {
    taggingRules: Object.values(taggingRulesRecord).map((taggingRule) => ({
      ...taggingRule,
