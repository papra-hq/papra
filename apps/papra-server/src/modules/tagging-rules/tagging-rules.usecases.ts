...
export async function applyTaggingRuleToExistingDocuments({
  ...
  // Fetch all non-deleted documents for the organization
  const { documents } = await documentsRepository.getAllOrganizationDocumentsForTagging({ organizationId });

  logger.info({ organizationId, taggingRuleId, documentCount: documents.length }, 'Starting to apply tagging rule to existing documents');

  // Use pLimit for concurrent processing
  ...
  await Promise.all(
    documents.map(async document => limit(async () => {
      const [, error] = await safely(async () => {
        ...
        // Count as tagged if the document matched any rule
        // (applyTaggingRules processes all enabled rules, but we can check if tags were added)
        taggedCount++;
      });
