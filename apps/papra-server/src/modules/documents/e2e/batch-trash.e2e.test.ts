    ...
    test('batch-trashes documents by id, leaving non-targeted documents untouched', async () => {
      ...
      const response = await app.request(
        `/api/organizations/${ORGANIZATION_ID}/documents/batch/trash`,
        {
          ...
          body: JSON.stringify({
            filter: {
              documentIds: ['doc_111111111111111111111111', 'doc_222222222222222222222222'],
            },
          }),
        },
    ...
    test('batch-trashes all documents matching a search query', async () => {
      const documents = [
        makeDocument({
          ...
          name: 'invoice-jan.txt',
          content: 'january invoice',
        }),
        makeDocument({
          id: 'doc_222222222222222222222222',
          name: 'invoice-feb.txt',
          content: 'february invoice',
        }),
        makeDocument({
