...
import { generateOrganizationId } from '../../../organizations/organizations.models';
...
export function buildStorageKey({ storageKeyPattern, ...context }: { storageKeyPattern: string } & StoragePatternInterpolationContext) {
  const storageKey = storageKeyPattern.replace(/\{\{(.*?)\}\}(?!\})/g, (match, rawExpression) => {
    ...
    return transformerParts.reduce((value, transformerPart) => {
      ...
      const argumentsString = transformerArgsSplitted.join(' ').trim();
