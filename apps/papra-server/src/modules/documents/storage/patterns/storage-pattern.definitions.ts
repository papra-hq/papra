...
export const expressionsDefinitions: Record<string, (context: StoragePatternInterpolationContext) => string> = {
  'document.id': context => context.document.id,
  'document.name': context => context.document.name,
  'document.createdAt': context => context.document.createdAt.toISOString(),
  'organization.id': context => context.organization.id,
  ...
  ...[
    ...
    ...acc,
    [`document.createdAt.${token}`]: (context: StoragePatternInterpolationContext) => formatDate(context.document.createdAt, `{${token}}`),
