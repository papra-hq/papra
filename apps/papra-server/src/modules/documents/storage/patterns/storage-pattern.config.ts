...
export const storagePatternConfig = {
  useLegacyStorageKeyDefinitionSystem: {
    doc: 'Whether to use the legacy storage key definition system, which generates storage keys in the format: {{organization.id}}/originals/{{document.id}}. When set to true, not storage key pattern will be used, nor will the incremental suffix or random suffix fallback mechanisms be enabled, as the storage key will be generated using the legacy system.',
  ...
  storageKeyPattern: {
    doc: 'The pattern to use for generating storage keys. This can include expressions enclosed in double curly braces (e.g. {{timestamp}}) that will be evaluated at runtime.',
    schema: z
      .string()
