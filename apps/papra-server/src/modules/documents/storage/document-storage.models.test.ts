...
describe('document-storage models', () => {
  describe('addSuffixToFileName', () => {
    test('given a storage key and a suffix, add the suffix before the file extension', () => {
      ...
      expect(addSuffixToFileName({ storageKey: 'archive.tar.gz', suffix: 'backup' })).to.eql(
    ...
    test('if there is no file extension, add the suffix at the end of the storage key', () => {
      ...
      expect(addSuffixToFileName({ storageKey: 'archive', suffix: 'backup' })).to.eql(
        'archive_backup',
      );
    });
    ...
    test('filenames starting with a dot (e.g. .env) are supported', () => {
      ...
      expect(addSuffixToFileName({ storageKey: '.env.local', suffix: 'backup' })).to.eql(
        '.env_backup.local',
      );
    });

    test('the storage key path can contain directories, the suffix should be added to the file name only', () => {
      expect(addSuffixToFileName({ storageKey: 'path/to/file.txt', suffix: '1' })).to.eql(
        'path/to/file_1.txt',
      );
      expect(
        addSuffixToFileName({ storageKey: 'path/dir.with.dots/file.txt', suffix: '2' }),
