...
export async function createIntakeEmail({
  ...
  const { username } = await intakeEmailUsernameServices.generateIntakeEmailUsername({
    userId,
    organizationId,
  });
...
export async function ingestEmailForRecipient({
  ...
  const { intakeEmail } = await intakeEmailsRepository.getIntakeEmailByEmailAddress({
