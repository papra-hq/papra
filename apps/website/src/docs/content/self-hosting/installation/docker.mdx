...
import RandomStringGenerator from '../../../components/RandomStringGenerator.astro'
...
<Tabs>
  <Tab label="Key generator">
    <RandomStringGenerator length="96" type="hexadecimal" />
  </Tab>
  <Tab label="OpenSSL command">
    ```bash
    openssl rand -hex 48
    ```
  </Tab>
...
Then launch Papra with default configuration using:
...
2. Launch Container with Volume Binding
    ...
    Make sure you have generated an `AUTH_SECRET` (see the [Quick Deployment](#quick-deployment) section above), then run:
