...
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from '@/modules/ui/components/dialog';
      ...
      <DialogContent class="sm:max-w-[480px]">
        ...
        <Show when={props.open}>
          <div class="flex flex-col gap-4">
            <div class="flex flex-col gap-2">
              <label class="text-sm font-medium">{t('documents.list.batch.tags.dialog.add-label')}</label>
              <DocumentTagPicker
            ...
            <div class="flex flex-col gap-2">
              <label class="text-sm font-medium">{t('documents.list.batch.tags.dialog.remove-label')}</label>
              <DocumentTagPicker
                organizationId={props.organizationId}
                tagIds={[]}
            ...
            <Show when={overlap()}>
              <p class="text-sm text-red-500">{t('documents.list.batch.tags.dialog.overlap-error')}</p>
            </Show>
          </div>
        </Show>
        ...
        <DialogFooter>
          <div class="flex gap-2 justify-end flex-col-reverse sm:flex-row">
            <Button variant="secondary" onClick={() => props.onOpenChange(false)} disabled={props.isPending}>
              {t('documents.list.batch.tags.dialog.cancel')}
            </Button>
            <Button onClick={handleSubmit} disabled={!canSubmit() || props.isPending} isLoading={props.isPending}>
