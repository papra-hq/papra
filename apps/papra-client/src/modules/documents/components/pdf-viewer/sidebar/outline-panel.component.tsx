    ...
    <li class="relative p-1 py-px">
      <div class="flex items-start">
        {hasChildren() ? (
          ...
          >
            <div
              class={cn('i-tabler-chevron-right size-3.5 transition-transform', {
                'rotate-90': getIsExpanded(),
              })}
            />
          </Button>
        ) : (
      ...
      <Show when={hasChildren() && getIsExpanded()}>
        <div class="ml-3 border-l">
          ...
          <OutlineItems outline={props.items ?? []} store={props.store} level={props.level + 1} />
        </div>
      </Show>
    </li>
  );
...
const OutlineItems: Component<{
  ...
  return (
    <ul class="w-full">
      <For each={getItems()}>
        {(item) => <OutlineItem {...item} store={props.store} level={props.level ?? 0} />}
      </For>
    </ul>
  );
};
...
export const OutlinePanel: Component<OutlinePanelProps> = (props) => {
  return (
    <div class="pt-2 text-foreground text-sm">
      <OutlineItems outline={props.store.documentOutline} store={props.store} />
    </div>
  );
};
