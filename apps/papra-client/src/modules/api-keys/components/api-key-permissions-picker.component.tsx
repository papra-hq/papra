...
export const ApiKeyPermissionsPicker: Component<{
  permissions: string[];
    ...
    return API_KEY_PERMISSIONS.map((section) => ({
      ...
      permissions: section.permissions.map((permission) => {
        ...
        return {
          ...
          suffix,
          description: t(
            `api-keys.permissions.${section.section}.${permission}` as TranslationKeys,
          ),
        };
      }),
    }));
    ...
    <div class="p-6 pb-8 border rounded-md mt-2">
        ...
        <For each={getPermissionsSections()}>
          {(section) => (
            <div>
              <Button
                variant="link"
                class="text-muted-foreground text-xs p-0 h-auto hover:no-underline"
      ...
      <div class="flex items-center gap-2 mt-6 border-t pt-6">
        <Button
          ...
          onClick={() =>
            setPermissions(API_KEY_PERMISSIONS.flatMap((section) => section.permissions))
          }
          disabled={
            permissions().length ===
            API_KEY_PERMISSIONS.flatMap((section) => section.permissions).length
          }
        >
