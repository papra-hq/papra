...
const TaggingRuleCard: Component<{ taggingRule: TaggingRule }> = (props) => {
  ...
  const applyRuleMutation = useMutation(() => ({
    ...
    onSuccess: (data) => {
      createToast({
        message: t('tagging-rules.apply.success', { count: data.taggedCount }),
        type: 'success',
      });
      queryClient.invalidateQueries({ queryKey: ['organizations', props.taggingRule.organizationId, 'documents'] });
    },
    onError: () => {
      createToast({
