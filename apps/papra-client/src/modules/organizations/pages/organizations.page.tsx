  ...
  return (
    <div class="p-6 mt-4 pb-32 max-w-5xl mx-auto">
      <div class="flex items-start justify-between mb-2">
        <div class="flex-1">
          <h2 class="text-xl font-bold">{t('organizations.list.title')}</h2>
        </div>
      ...
      <p class="text-muted-foreground mb-6">{t('organizations.list.description')}</p>
      ...
      <div class="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <For each={query.data?.organizations}>
          {(organization) => (
            <A href={`/organizations/${organization.id}`} class="border rounded-lg overflow-hidden">
              ...
              <div class="p-4">
                <div class="w-full text-left font-bold truncate block">{organization.name}</div>
              </div>
            </A>
          )}
        </For>
        ...
        <A
          ...
          class="border rounded-lg overflow-hidden border-dashed border-2px p-4 flex flex-col items-center justify-center text-center gap-2 group"
        >
